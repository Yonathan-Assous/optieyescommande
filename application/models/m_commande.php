    ...
    public function CAMensuel($date, $tarif_Livraison = true, $is_teledetourage = false){
        ...
        if($tarif_Livraison){
            $query = $this->db->query("SELECT tarif_livraison as TarifLivraison,id_users,date_update_commande as date_and_time_update_commande, date(date_update_commande) as date_update_commande,DATE_FORMAT(date_update_commande, '%e') as day
                                     ...
                                     AND tarif_livraison > 0
                                     AND id_etat_commande = 6
                                     AND ( DATE_FORMAT(date_commande, '%Y-%m') = '".$date."' AND ( (date_update_commande < '2015-07-19') OR (TIME(date_and_time_update_commande) >= '09:00:00' AND TIME(date_and_time_update_commande) < '16:00:00')))
...
//        var_dump($sql);die;
    ...
    public function getCaJournalier($tarif_Livraison = true, $is_teledetourage = false) {
        ...
        if($tarif_Livraison){
            ...
            $sql = "SELECT COALESCE(TarifLivraison1,0) + COALESCE(TarifLivraison2,0) as TarifLivraison,id_users,date_update_commande
                                     FROM ".$this->table." c
									 LEFT JOIN 
									  ( 
										SELECT tarif_livraison as TarifLivraison1, id_commande as idCommande
...
//            print_r($sql);
    ...
    public function getCaByDayOfMonth($date){
        ...
        $sql = "SELECT tarif_livraison as TarifLivraison, DATE_FORMAT(date_update_commande, '%e') AS day, id_users, date(date_update_commande) AS date_update_commande
                                   FROM ".$this->table."
                                   WHERE DATE_FORMAT(date_update_commande, '%Y-%m')='".$date."'
                                   AND tarif_livraison > 0
                                   AND id_etat_commande = 6
        ...
        $sql = "SELECT COALESCE(SUM(TarifLivraison1),0) + COALESCE(SUM(TarifLivraison2),0) as TarifLivraison, DATE_FORMAT(date_update_commande, '%e') AS day, id_users, date(date_update_commande) as date_update_commande
                                     FROM ".$this->table." c
									 LEFT JOIN 
									  ( 
										SELECT tarif_livraison as TarifLivraison1, id_commande as idCommande
...
//    public function getTotalCommandePerUSersByMonth()
//    {
//        $sql = "SELECT id_users, SUM(`total_commande`) as total FROM `commande`
//                WHERE `date_commande` >= '2021-12'
//                AND (type_commande = 1
//                OR (type_commande > 1 AND penalty = 1))
//                GROUP BY id_users";
//        $query = $this->db->query($sql);
