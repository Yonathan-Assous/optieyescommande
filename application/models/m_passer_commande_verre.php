...
class m_passer_commande_verre extends CI_Model {
	...
	public function getlensFocalGroups()
	{
	...
	public function delCustomPrice($id_prix)
	{
		if($id_prix!="")
		{
	...
	public function getCustomPriceList($user_id)
	{
		$tab = array();
		...
		if($user_id != "")
		{
			...
			foreach( $query as $r ) 
			{
				...
				$tab[$i]['prix'] = $r->prix;
				$tab[$i]['id'] = $r->id;
				$tab[$i]['action'] = '<a class="modifier_prix btn btn-icon waves-effect waves-light btn-warning tooltipster" href="#" rel="'.$r->id.'*'.$r->prix.'" original-title="Modifier" title="Modifier" >Modifier</a> <a class="supprimer_prix btn btn-icon waves-effect waves-light btn-warning tooltipster" href="#" rel="'.$r->id.'" original-title="Supprimer" title="Supprimer" >Supprimer</a>';
	...
	public function setPriceVerreById($new_price,$id_prix)
	{
		if($id_prix!="" && $new_price!="")
		{
			$res = $this->db->query("UPDATE prix_par_client set prix='".$new_price."' WHERE id = '".$id_prix."'");
			//$query = $res->result();
	...
	public function setPriceVerre($user_id,$new_price,$code_verre,$name_verre)
	{
    	
    	if($user_id!="" && $new_price!="" && $code_verre!="")
		{
			$generation = "";
			...
			$res = $this->db->query("INSERT INTO `prix_par_client` (`id`, `code`, `id_client`, `prix`, name, generation) VALUES (NULL, '".$code_verre."', '".$user_id."', '".$new_price."', '".addslashes($name_verre)."', '".$generation."')");
			//$query = $res->result();
	...
	public function getListeClients(){
     ...
     public function duplicatePriceTab($user_id,$from_user_id){
    	...
    	if($user_id != "" && $from_user_id != "")
		{
			...
			{
				$res = $this->db->query("INSERT INTO `prix_par_client` (`id`, `code`, `id_client`, `prix`, `name`, `generation`) VALUES (NULL, '".$from->code."', '".$user_id."', '".$from->prix."', '".addslashes($from->name)."', '".$from->generation."')");
	...
	public function getAllLenses($requete = "")
	{
		$tab = array();
	...
	public function getstocklensAdmin($indice,$lensFocalGroup,$generation,$sphereD,$cylindreD,$axeD,$additionD,$stock,$user_id,$panierA)
	{
		...
		if(($lensFocalGroup == "3" && $indice == "1.5") && ($sphereD >= -3 && $sphereD <= 2) && ($additionD >= 1 && $additionD <= 3))
		{
			//26783 diametre 70
		...
		else
		{
			...
			$stock_res = $this->db->query("SELECT *, ppc.prix as prix_perso  
									   ...
									   JOIN grille_tarifaire ON grille_tarifaire.id_verre = verres_stock.id_verre
                                       LEFT JOIN prix_par_client ppc ON (ppc.code = verres_stock.id_verre AND id_client=".$user_id.")
									   WHERE libelle_verre LIKE '% ".$indice_fr." %' AND id_grille_tarifaire = 1 ".$P_A."
									   ORDER BY libelle_verre ASC");
	...
	public function getstocklens($indice,$lensFocalGroup,$generation,$sphereD,$cylindreD,$axeD,$additionD,$stock,$user_id,$panierA,$type="1")
	{
	...
	public function getlens($indice,$lensFocalGroup,$generation,$sphereD,$cylindreD,$axeD,$additionD,$stock,$user_id,$panierA,$type="1")
	{
		...
		if($sphereD!="-" && $cylindreD!="-")
		{
			...
			else
			{
				...
				if($lensFocalGroup == '4' && $additionD > 2.25)
				{
					$sphereD_res = $this->db->query("SELECT * 
									   FROM ".$this->table_Refractions." 
									   WHERE id = 0
					...
					for($i=0;$i<sizeof($tab_ranges);$i++)
					{
						if($tab_ranges[$i] != "")
						{
							...
							/*$ranges_res = $this->db->query("SELECT * 
										   ...
										   WHERE id=".$tab_ranges[$i]."
										   ");*/
							if ($ranges_res && $ranges_res->num_rows() > 0)
							{
								$ranges[]=$tab_ranges[$i];
							}
	...
	WHERE l2.sorting IS NULL AND  (".$codes_f." L.code = '0') AND L.trad_fr LIKE '%T-One%' ORDER BY sorting,trad_fr");	
	...
	FROM lenses L LEFT JOIN lenses l2
	 ON (L.trad_fr = l2.trad_fr AND L.sorting > l2.sorting AND (L.name LIKE '%trn%' OR L.name LIKE '%transition%'  OR L.name LIKE '%chrom%'))
	WHERE l2.sorting IS NULL AND  (".$codes_f." L.code = '0') AND L.trad_fr LIKE '%E-Space%' ORDER BY sorting,trad_fr");	
	...
	public function getlensAdminAll($indice,$lensFocalGroup,$generation,$sphereD,$cylindreD,$axeD,$additionD,$stock,$user_id,$panierA)
	{
	...
	public function getlensAdmin($indice,$lensFocalGroup,$generation,$sphereD,$cylindreD,$axeD,$additionD,$stock,$user_id,$panierA)
	{
	...
	public function getPrixStock($lens="",$user_id)
	{
		$stock_res = $this->db->query("SELECT *, ppc.prix as prix_perso  
												FROM verres_stock 
									   JOIN grille_tarifaire ON grille_tarifaire.id_verre = verres_stock.id_verre
                                       LEFT JOIN prix_par_client ppc ON (ppc.code = verres_stock.id_verre AND id_client=".$user_id.")
									   WHERE verres_stock.id_verre = '".$lens."' AND grille_tarifaire.id_grille_tarifaire = 1");
	...
	public function getPrix($lens="",$user_id,$generation)
	{
		...
		if($generation != "")
		{
			...
			FROM lenses L 
			LEFT JOIN prix_par_client ppc ON (ppc.code = L.code AND id_client=".$user_id." AND ppc.generation LIKE '%".$generation."%')
	...
	public function getColors($lens="")
	{
		if(strpos($lens, 'Mineral') !== false || strpos($lens, 'Minéral') !== false)
		{
			$res = $this->db->query("SELECT * FROM lensOptions WHERE color='COLOR' AND display='X' AND (trad_fr LIKE '%gris%' OR trad_fr LIKE '%brun%') AND trad_fr NOT LIKE '%dégradé%'  AND trad_fr NOT LIKE '% D %'  AND trad_fr NOT LIKE '%nm%' ORDER BY sorting");
		}
		else
		{
	...
	public function getColorsWithPerso($lens="")
	{
		if(strpos($lens, 'Mineral') !== false || strpos($lens, 'Minéral') !== false)
		{
			$res = $this->db->query("SELECT * FROM lensOptions WHERE (color='COLOR' AND display='X' AND (trad_fr LIKE '%gris%' OR trad_fr LIKE '%brun%') AND trad_fr NOT LIKE '%dégradé%'  AND trad_fr NOT LIKE '% D %'  AND trad_fr NOT LIKE '%nm%') OR code = 'CUST_24' OR code = 'CUST_25' ORDER BY sorting");
		}
		else
		{
			$res = $this->db->query("SELECT * FROM lensOptions WHERE (color='COLOR' AND display='X') OR code = 'CUST_24' OR code = 'CUST_25' ORDER BY sorting");
		}
		return $res->result();
	}
	
	public function getDiametres($lens,$sphere,$cylindre)
	{
		...
		$res_ranges = $this->db->query("SELECT * FROM lensRanges WHERE id IN (".$ranges_listF.") ORDER BY diameter_physical ASC ");
		...
		foreach( $ranges as $range ) 
		{
			...
			for($i=0;$i<sizeof($refr)-1;$i++)
			{
				...
				$res = $this->db->query("SELECT * FROM Refractions WHERE id = $refr[$i] AND maxMeridian_from <= $sphere  AND maxMeridian_to >= $sphere AND cylinder_from <= $cylindre AND cylinder_to >= $cylindre AND cylinderPart_from = '0'");
				$refrc = $res->result();
				...
				$res_100 = $this->db->query("SELECT * FROM Refractions WHERE id = $refr[$i]  AND maxMeridian_to >= $sphere AND cylinder_from <= $cylindre AND cylinder_to >= $cylindre AND cylinderPart_from = '100'");
					...
					if($from == '100' && $to == '0')
					{
						...
						$s=$ref100->maxMeridian_from;
							...
							for($k=$ref100->cylinder_from;$k<=$ref100->cylinder_to; $k += 0.25)
							{
								...
								if($k == $cylindre && $s <= $sphere )//&& $sphere <= $ref100->maxMeridian_from  )
								{
									//echo "LA3<br>";
									//$valid_cylindre = $s;
		...
		$rangesids = rtrim($rangesids," OR ");
					...
					//	echo $rangesids."<br>";
						...
						$resultats = $this->db->query("SELECT * FROM lensRanges WHERE ".$rangesids."  GROUP BY diameter_physical ORDER BY diameter_physical");
	...
	public function getOptions($lens)
	{
		/*
		...
		*/
		...
		if($p[0] != "2")
		{
			...
			$tree = $this->db->query("SELECT * FROM combinationTrees JOIN combinationNodes ON combinationNodes.id = combinationTrees.combinationNodeId WHERE combinationTrees.id = ".$p[0]." ");
		//	echo "SELECT * FROM combinationTrees JOIN combinationNodes ON combinationNodes.id = combinationTrees.combinationNodeId WHERE combinationTrees.id = ".$p[0]." ";
			...
			if($tree->result()[0]->id != "" && $tree->result()[0]->childsIds != "")
			{
				
				//$tree2 = DB::table("combinationNodes")->where('combinationNodes.id', '=', $tree[0]->childsIds)->get();
				$tree2 = $this->db->query("SELECT * FROM combinationNodes WHERE combinationNodes.id = ".$tree->result()[0]->childsIds." ");
				...
				//	echo "<br><br>".$optionCode2;
					...
					if($tree2->result()[0]->id != "")
					{
						...
						$tree3 = $this->db->query("SELECT * FROM combinationNodes WHERE combinationNodes.id = ".$tree2->result()[0]->id." ");
						...
						if($tree3->result()[0]->id != "")
						{
							...
							$tree4 = $this->db->query("SELECT * FROM combinationNodes WHERE combinationNodes.id = ". $tree3->result()[0]->id." ");
			...
			$optionList = $this->db->query("SELECT * FROM lensOptions WHERE code IN ('".$codes."') AND display='X' AND color!='COLOR' GROUP BY trad_fr ORDER BY sorting ");
