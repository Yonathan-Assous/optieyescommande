...
class m_passer_commande_verre extends CI_Model {
	...
	public function setPriceVerre($user_id,$new_price,$code_verre)
	{
    	
    	if($user_id!="" && $new_price!="" && $code_verre!="")
		{
			$res = $this->db->query("INSERT INTO `prix_par_client` (`id`, `code`, `id_client`, `prix`) VALUES (NULL, '".$code_verre."', '".$user_id."', '".$new_price."')");
			//$query = $res->result();
	...
	public function getListeClients(){
     ...
     public function duplicatePriceTab($user_id,$from_user_id){
    	...
    	if($user_id != "" && $from_user_id != "")
		{
			...
			{
				$res = $this->db->query("INSERT INTO `prix_par_client` (`id`, `code`, `id_client`, `prix`) VALUES (NULL, '".$from->code."', '".$user_id."', '".$from->prix."')");
	...
	public function getstocklens($indice,$lensFocalGroup,$generation,$sphereD,$cylindreD,$axeD,$additionD,$stock,$user_id,$panierA)
	{
	...
	public function getlens($indice,$lensFocalGroup,$generation,$sphereD,$cylindreD,$axeD,$additionD,$stock,$user_id,$panierA)
	{
	...
	FROM lenses L LEFT JOIN lenses l2
	 ...
	 LEFT JOIN prix_par_client ppc ON (ppc.code = L.code AND id_client=".$user_id.")
	WHERE l2.sorting IS NULL AND  (".$codes_f." L.code = '0') AND L.trad_fr LIKE '%T-One%' ORDER BY sorting,trad_fr,prix");	
	...
	FROM lenses L LEFT JOIN lenses l2
	 ON (L.trad_fr = l2.trad_fr AND L.sorting > l2.sorting AND (L.name LIKE '%trn%' OR L.name LIKE '%transition%'  OR L.name LIKE '%chrom%'))
	 LEFT JOIN prix_par_client ppc ON (ppc.code = L.code AND id_client=".$user_id.")
	WHERE l2.sorting IS NULL AND  (".$codes_f." L.code = '0') AND L.trad_fr LIKE '%E-Space%' ORDER BY sorting,trad_fr,prix");	
					...
					elseif($generation == "Platinium")
					{
						$res_f = $this->db->query("SELECT L.code,L.id, L.name, L.trad_fr, L.prix, ppc.prix as prix_perso , L.sorting
														FROM ".$this->table_lenses." L 
														LEFT JOIN prix_par_client ppc ON (ppc.code = L.code AND id_client=".$user_id.")
											   WHERE (".$codes_f." L.code = '0') ORDER BY sorting,trad_fr,prix");
	...
	FROM lenses L LEFT JOIN lenses l2
	 ...
	 LEFT JOIN prix_par_client ppc ON (ppc.code = L.code AND id_client=".$user_id.")
	WHERE l2.sorting IS NULL AND  (".$codes_f." L.code = '0') ".$P_A." ORDER BY sorting,trad_fr,prix");	
