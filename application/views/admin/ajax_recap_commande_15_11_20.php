    ...
    foreach($orderbag as $k => $recap_commande) {
       ...
       if($k == 'order' && isset($orderbag['pair_order'])) {
		...
		if(($panierA==1 || $panierA==$user_info->id_users) && $montures->modele!='')
            ...
            if(isset($recap_commande['PrismeD']) && $recap_commande['PrismeD']=='1')
			{
				$correction_droit  .= '<b>Prisme sphere :</b>'.$recap_commande['PrismeSphereD']."<br>";
				$correction_droit  .= '<b>Prisme cylindre :</b>'.$recap_commande['PrismeCylindreD']."<br>";
			}
			
			if(isset($recap_commande['PrismeG']) && $recap_commande['PrismeG']=='1')
			{
				$correction_gauche  .= '<b>Prisme sphere :</b>'.$recap_commande['PrismeSphereG']."<br>";
				$correction_gauche  .= '<b>Prisme cylindre :</b>'.$recap_commande['PrismeCylindreG']."<br>";
			}
             ...
             /*   if(isset($info_commande['verre']['polarise'])){
               ...
               /* if($recap_commande['type_commande'] == 1 || $recap_commande['type_commande'] == 2)
                ...
                if($k == 'pair_order') {
                	...
                	if($info_commande['verre']['correction_droit']['teinte'])
                	{
                		$info_d .= '<b>Teinte :</b> ';
...
*/
                ...
                */
                if($k == 'pair_order') {
                	...
                	if($info_commande['verre']['correction_gauche']['teinte'])
                	{
                		$info_g .= '<b>Teinte :</b> ';
                	...
                	echo '<td><h4>Oeil gauche : '.$recap_commande['trad_fr'].'</h4>'.$correction_gauche.'</td>';
                	echo '<td class="center">1</td>';
                ...
                else
                {
					...
					if($user_info->restriction_compte == 0){
						...
						if($recap_commande['type_de_verreG']!="")
						{
							...
							if(isset($discount) && $discount['on'] == $k) {
								if($discount['amount'] > 0) {
									$recap_commande['prixGH'] = ($recap_commande['prixGH']) - ( ( ($recap_commande['prixGH']) / 100 ) * $discount['amount'] );
...
if(isset($recap_commande['user_info']->show_commentaire) && $recap_commande['user_info']->show_commentaire == '1') {
			?>
