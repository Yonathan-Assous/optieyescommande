    ...
    <div class="content">
        <div class="container">
            ...
            <div class="row">
                ...
                <div class="col-sm-12">
                    <h4 class="page-title m-b-20">Prix des montures</h4>
                </div>
                ...
                <div class="col-sm-12">
                    <div class="card-box">
                        ...
                        <h4 class="header-title m-t-0 m-b-30">Liste des montures</h4>

                        <table id="datatable" class="table table-montures table-striped dt-responsive nowrap">
                            <thead>
                            <tr>
                                <th>Montures</th>
                                <th>Prix</th>
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#datatable').DataTable({
                ajax: { url: "/admin/prix_montures_ajax/<?php echo $id_generation; ?>", dataSrc: 'aaData' },
                deferRender: true,
                ordering: false,
                language: {
                    "lengthMenu": "Afficher _MENU_ montures par page",
                    "zeroRecords": "Aucun monture trouvé",
                    "info": "Affichage de la page page _PAGE_ sur _PAGES_",
                    "infoEmpty": "Aucune monture à afficher",
            ...
            $(".content").on("change", "#grille_tarifaire", function(){
                table.ajax.url('/admin/prix_montures_ajax/<?php echo $id_generation; ?>/'+$(this).val()).load();
            });
            ...
            $(".content").on("keyup","input[name^=prix_]", function(){
                if($.isNumeric($(this).val())){
                    ...
                    var value = $.trim($(this).val()), id_montures = $(this).attr("id").split("_")[1], id_grille_tarifaire = $(this).attr("id").split("_")[2];
                    ...
                    Timer = setTimeout( function( ) {
                        $.ajax({
                            type: "POST",
                            url: "/admin/updatePrixMontures",
                            data : "id_montures="+id_montures+"&id_grille_tarifaire="+id_grille_tarifaire+"&prix_montures="+value,
                            success: function(data){
                                swal({
                                    title: "Tarif mis à jour",
                                    text: "Le prix de la monture a été mis à jour.",
                                    type: "success",
            ...
            $(".content").on("click","button[id^=supprimer_]", function(){
                if(confirm('Supprimer cette monture ?')){
                    var id_montures = $(this).attr('id').split('_')[1];
                    $.ajax({
                        type: "POST",
                        url: "/admin/desactive_montures",
                        data : "id_montures="+id_montures,
                        success: function(data){
            ...
            $(".content").on("keyup","input[id^=order_]", function(){
                ...
                if($.isNumeric(value)){
                    var id_montures = $(this).attr("id").split("_")[1], id_grille_tarifaire = $(this).attr("id").split("_")[2];
                    $.ajax({
                        type: "POST",
                        url: "/admin/ordre_montures",
                        data : "id_montures="+id_montures+"&id_grille_tarifaire="+id_grille_tarifaire+"&order_montures="+value,
                        success: function(data){
                            swal({
                                title: "Ordre mis à jour",
                                text: "La position de la monture a été mise à jour.",
                                type: "success",
                                showCancelButton: false,
                                showConfirmButton: false,
                                timer: 2000,
