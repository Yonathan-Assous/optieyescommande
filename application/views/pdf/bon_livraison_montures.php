                ...
                foreach($montures as $monture) {
					if($monture['modele']!=null)
					{
						echo '<tr>
								...
								<td align="center">'.$monture['qty'].'</td>
							  </tr>';
					}
					else
					{
						$montures_du_pack = $this->m_montures->getAllMonturesDuPack($monture['pack_id']);
						 foreach($montures_du_pack as $monture_du_pack)
						 {
							...
							$monture['collection']=$this->m_montures->getCollectionName($monture_du_pack->id_collection)->name; 
							 echo '<tr>
								<td>
									<h4>'.$monture['collection'].' '.$monture_du_pack->modele.'</h4><br>
									Taille : '.$monture_du_pack->size.'<br>
									Type : '.$monture_du_pack->type.'<br>
									Couleur : '.$monture_du_pack->couleur.'
								</td>
								<td>'.$monture['qty'].'</td>
							  </tr>';
						 }
