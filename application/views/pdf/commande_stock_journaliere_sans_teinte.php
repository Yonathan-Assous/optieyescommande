                ...
                if(is_array($commande_journaliere)){
                  ...
                  //  var_dump($commande_journaliere);
                    ...
                    foreach($commande_journaliere as $key => $value){
						...
						{
							if($value->generation_verre=="Progressif")
							{
								$value->libelle_verre = "Prog Eco 1,5 HMC";
                       ...
                       $info_commande = json_decode($value->information_commande, true);
						//echo $value->generation_verre."<br>";
						//if($info_commande['verre']['teinte']=="Non Teinté" || $info_commande['verre']['teinte']=="")
						if ($value->generation_verre != "Stock teinté" || $value->generation_verre == "")//(strpos($value->libelle_verre, 'Polarisé') !== false )    generation_verre
						{
                       ...
                       if($value->generation_verre=="Progressif")
						{
							...
							$value->traduction = "Prog Eco 1,5 HMC";
						}
                       
                       if($envoi_chine == 1){


                         if(strpos($value->traduction, '#diametre#') !== false){
                         	if(isset($info_commande['verre']['correction_droit']['diametre']))
                         	{
                         		 $name = str_replace('#diametre#',$info_commande['verre']['correction_droit']['diametre'], $value->traduction);
                         	}
                         	else
                         	{
								if(isset($info_commande['verre']['diametre']))
								  $name = str_replace('#diametre#',$info_commande['verre']['diametre'], $value->traduction);
							}
							
							if(isset($info_commande['verre']['correction_gauche']['diametre']))
                         	{
                         		 $name_gauche = str_replace('#diametre#',$info_commande['verre']['correction_gauche']['diametre'], $value->traduction);
                         	}
                         	else
                         	{ 
								if(isset($info_commande['verre']['diametre_gauche']))
								  $name_gauche = str_replace('#diametre#',$info_commande['verre']['diametre_gauche'], $value->traduction);
							}
							
                         } else {
                            if(isset($info_commande['verre']['correction_droit']['diametre']))
                         	{
                         		 $name = $value->traduction." ".$info_commande['verre']['correction_droit']['diametre'];
                         	}
                         	else
                         	{
								if(isset($info_commande['verre']['diametre']))
								  $name = $value->traduction." ".$info_commande['verre']['diametre'];
							}
                            
                            if(isset($info_commande['verre']['correction_gauche']['diametre']))
                         	{
                         		 $name_gauche = $value->traduction." ".$info_commande['verre']['correction_gauche']['diametre'];
                         	}
                         	else
                         	{
								if(isset($info_commande['verre']['diametre_gauche']))
								  $name_gauche = $value->traduction." ".$info_commande['verre']['diametre_gauche'];
							}
                         }
               ...
               //     var_dump($list_verre);
                    ...
                    foreach($list_verre as $verre => $list){
                         ...
                         if($verre == 'Prog Eco 1,5 HMC 70' || $verre == 'PROGRESSIF LENSES 1,56 HMC (CORRIDOR 14MM) 70') {
                          	$verre = 'PROGRESSIF LENSES 1,56 HMC (CORRIDOR 14MM) 70';
                          }
