		...
		<tr>
			...
			if(isset($etiquette) && $etiquette !== false){
				...
				foreach($etiquette as $value){
					...
					echo '<td style="width:266px;min-width:266px;height:'.$height.'px;">
								<table style="width:266px;min-width:266px;height:'.$height.'px;'.($margin_left ? 'margin-left:-10px;' : '').' '.($margin_center ? 'margin-left:-10px;' : '').' '.($margin_right ? 'margin-left:-10px;' : '').' margin-top:-23px">';
					echo '<tr>
									<td>';
					...
					else
					{
						...
						{
								$value->libelle_verre .= " ".utf8_decode($this->m_commande->getTraitementByCode($info_commande['verre']['correction_gauche']['traitement']));
						}
					...
					echo '<tr>
									<td style="text-align:center;font-weight:bold;font-size:14px;width:100%;">
										'.($info_commande['verre']['correction_'.$value->cote]['sphere']>0 ? '+' : '').number_format($info_commande['verre']['correction_'.$value->cote]['sphere'], 2, ',', ' ').($displayCylindre ? '('.$sign.number_format($info_commande['verre']['correction_'.$value->cote]['cylindre'], 2, ',', ' ').') '.(isset($info_commande['verre']['correction_'.$value->cote]['axe']) ? $info_commande['verre']['correction_'.$value->cote]['axe']."°" : "") : '' ).' '.((isset($info_commande['verre']['correction_'.$value->cote]['addition']) && $value->id_generation_verre != 34) ? " ADD ".number_format($info_commande['verre']['correction_'.$value->cote]['addition'], 2, ',', ' ') : "").((isset($info_commande['verre']['correction_'.$value->cote]['degression']) || $value->id_generation_verre == 34) ? " DEG ".number_format($info_commande['verre']['correction_'.$value->cote]['addition'], 2, ',', ' ') : "").'
									</td>
								   </tr>';
					echo '<tr>
									<td style="text-align:right;">
