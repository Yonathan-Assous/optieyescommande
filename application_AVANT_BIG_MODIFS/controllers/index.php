...
class index extends MY_Controller {
	...
	public function test_mail(){
		...
		$subject = "test";
		$txt = "Ca marche?";
		$headers = "From: crystal@optieyescommande.com";
        ...
        if($this->session->userdata('logged_in') === true && $this->input->is_ajax_request()){
            ...
            if($orders) {
                ...
                foreach($orders as $order)  {
					...
					if($order->id_etat_commande>1)
						$bl = '<a href="/index/generer_pdf/bon_livraison_lentille/' . $order->id_commande . '" class="btn btn-sm btn-warning"><i class="zmdi zmdi-download"></i> '. $order->intitule_bl.'</a>';
					else
						$bl = '';
	...
	public function bons_livraison_ajax(){
        if($this->session->userdata('logged_in') === true && $this->input->is_ajax_request()){
            ...
            $data_bon_livraison = $this->m_commande->getCommandesBL($dataOrder);
    ...
    public function recommande($order_id = false) {
        ...
        if($this->session->userdata('logged_in') === true){
            ...
            if($order = $this->m_commande->getCommandeById($order_id, $this->data['user_info']->id_users, false)) {
                ...
                if($this->data['user_info']->commande_suspendue == 0 && $this->data['user_info']->valid_mandat == 1)
        		{
