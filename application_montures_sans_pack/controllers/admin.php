    ...
    */
    ...
    public function set_montures_order() {
        ...
        if($this->input->is_ajax_request()){
			...
			$this->db->select('m.*, cm.id as id_temp, cm.qty, cm.status, cm.id_client, collec.name as collection');
			$this->db->where('cm.id_client', $id_user);
			$this->db->where('cm.status', 0);
			$this->db->join('montures m', 'm.id = cm.id_monture');
			$this->db->join('montures_collection collec', 'm.id_collection = collec.id');
			$query = $this->db->get();
			if ($query->num_rows() > 0) {
