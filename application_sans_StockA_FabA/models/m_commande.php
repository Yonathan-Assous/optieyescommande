...
class m_commande extends CI_Model {
	...
	public function getCommandeByUserAvecMontures($data){
        ...
        if(isset($data['tri'])) {
            ...
            switch ($data['tri']) {
                case 'stock':
                    $add = "AND ((c.id_type_generation_verre = 5 OR c.id_type_generation_verre = 23) OR commande_monture = 1)";
                    break;
                case 'others':
                    $add = "AND ((c.id_type_generation_verre <> 5 AND c.id_type_generation_verre <> 23) OR commande_monture = 1)";
                    break;
                case 'pending':
        ...
        $query = $this->db->query("SELECT date_update_commande,c.id_commande,c.id_type_generation_verre,date_commande, c.tarif_express, c.id_etat_commande,information_commande,type_commande,ancienne_commande,total_commande,reference_client,libelle_etat_commande,IF(libelle_verre_personnalise <> 'NULL',libelle_verre_personnalise,generation_verre) as generation_verre,commande_monture,
    ...
    public function getCommandeByUser($data){
        ...
        if(isset($data['tri'])) {
            ...
            switch ($data['tri']) {
                case 'stock':
                    $add = "AND (c.id_type_generation_verre = 5 OR c.id_type_generation_verre = 23) ";
                    break;
                case 'others':
                    $add = "AND (c.id_type_generation_verre <> 5 AND c.id_type_generation_verre <> 23) ";
                    break;
                case 'pending':
                    $add = "AND c.id_etat_commande < 6";
                    break;
    ...
    function getAllCommandeByUser($id_users = false, $date = null) {
        ...
        if($id_users !== false) {
            ...
            $query = $this->db->query('SELECT c.id_commande, c.reference_client, v.libelle_verre, c.date_commande  FROM commande AS c LEFT JOIN verres AS v ON c.id_verre = v.id_verre WHERE id_users = '.$id_users.$dateQuery.' ORDER BY date_commande DESC');
    ...
    public function getTotalCommandeByYear($id_users){
        ...
        // Excluded lenses
        $query = $this->db->query("SELECT SUM(total_commande+tarif_livraison) AS total FROM commande WHERE id_users = ".$id_users." AND (type_commande = 1 OR (type_commande > 1 AND penalty = 1)) AND id_verre IS NOT null AND date_commande > '2016-01-01 00:00:00'");
        ...
        // version corrigée, finalement non..
        /*$annee = date('Y', time());
    ...
    public function getTotalECCommandeByYear($id_users){
        ...
        $year = date('Y', time());
        $query = $this->db->query("SELECT COALESCE(SUM(totalCommande),0) + COALESCE(SUM(totalExpress),0) AS total FROM commande c
    ...
    public function getAllCommandeByMonth($data){
        ...
        if ($query && $query->num_rows() > 0){
          foreach($query->result() as $result){
            ...
            $query = $this->db->query("SELECT l.name as lens_name, c.id_commande,c.id_users, c.type_commande, c.penalty, date_commande,c.tarif_express,reference_client,total_commande,generation_verre,taux_tva,libelle_verre_personnalise, 0 as TarifLivraison,intitule_bl, c.id_users, date_update_commande, tarif_packaging, c.commande_monture
                                       FROM ".$this->table." c
                                       LEFT JOIN generation_verre gv ON gv.id_generation_verre = c.id_generation_verre
    ...
    public function getSupplementByMonth($date, $user = null) {
        ...
        $query = $this->db->query('SELECT SUM(tarif_supplement) AS total_supplement FROM commande WHERE DATE_FORMAT(date_commande, "%m-%Y") = "'.$date.'" '.$add);
    ...
    public function getSupplementByDay($date = null) {
        ...
        $query = $this->db->query('SELECT SUM(tarif_supplement) AS total_supplement FROM commande WHERE date_commande > "'.$date.'" AND date_commande < "'.$date_end.'"');
        $total = 0;
    ...
    public function getPackagingByDay($date = null, $user = null) {
        ...
        $query = $this->db->query('SELECT tarif_packaging AS total, MAX(date_commande) FROM commande WHERE id_users IN('.implode(',',$new_orders).') AND date_commande > "'.$date.'" AND date_commande < "'.$date_end.'" GROUP BY id_users ORDER BY date_commande DESC');
    ...
    public function getAllCommandeByMonthAndUser($date, $excl = null, $user = null){
        ...
        $query = $this->db->query("SELECT COALESCE(total_stock, 0)  + COALESCE(total_fabrique, 0) + COALESCE(total_lentilles, 0) + COALESCE(total_montures, 0) + COALESCE(total_express, 0) - COALESCE(total_reductions, 0) as total,total_stock,total_fabrique,total_lentilles,total_montures, total_express, c.type_commande, c.tarif_express, c.penalty, c.date_commande,c.id_users, COALESCE(total_reductions, 0) as reduction,cp,nom_magasin,id_grille_tarifaire,nom_societe,intitule_bl, 0 as tarif_liv, COALESCE(exp_stock) AS expeditions_stock, COALESCE(exp_fabric) AS expeditions_fabric
          ...
          (SELECT 
            ...
            FROM ".$this->table." c
            WHERE (id_type_generation_verre = 5 OR id_type_generation_verre = 23) 
          ...
          (SELECT 
            ...
            FROM ".$this->table." c
            WHERE (id_type_generation_verre <> 5 AND id_type_generation_verre <> 23)
            AND (type_commande = 1 OR (type_commande > 1 AND penalty = 1))
            AND DATE_FORMAT(date_commande, '%m-%Y') = '".$date."'
	...
	public function getCAmonthSupplement_Samuel($date){
		...
		WHERE DATE_FORMAT(date_commande, "%m-%Y") = "'.$date.'" AND Samuel = 100.00');
	...
	public function getCAmonth_Samuel($date){
        ...
        $query = $this->db->query("SELECT (SELECT IFNULL(SUM(commande.total_commande),0) as ca_journalier FROM commande
								   JOIN users ON commande.id_users = users.id_users
                                   WHERE DATE_FORMAT(date_commande, '%Y-%m')='".$date."' AND type_commande = 1 AND Samuel = 100.00)
                                   +
	...
	public function getCAmonthSupplement_Daniel($date){
        ...
        $query = $this->db->query('SELECT SUM(commande.tarif_supplement) AS total_supplement FROM commande
		JOIN users ON commande.id_users = users.id_users
		WHERE DATE_FORMAT(date_commande, "%m-%Y") = "'.$date.'" AND Daniel = 100.00');
	...
	public function getCAmonth_Daniel($date){
        ...
        $query = $this->db->query("SELECT (SELECT IFNULL(SUM(commande.total_commande),0) as ca_journalier FROM commande
								   JOIN users ON commande.id_users = users.id_users
                                   WHERE DATE_FORMAT(date_commande, '%Y-%m')='".$date."' AND type_commande = 1 AND Daniel = 100.00)
                                   +
                                  (SELECT IFNULL(SUM(commande.total_commande),0) as ca_journalier_penalty FROM commande
								   JOIN users ON commande.id_users = users.id_users
	...
	public function getCAmonthSupplement_Gregory($date){
        ...
        $query = $this->db->query('SELECT SUM(commande.tarif_supplement) AS total_supplement FROM commande
		JOIN users ON commande.id_users = users.id_users
		WHERE DATE_FORMAT(date_commande, "%m-%Y") = "'.$date.'" AND Gregory = 100.00');
	...
	public function getCAmonth_Gregory($date){
        ...
        $query = $this->db->query("SELECT (SELECT IFNULL(SUM(commande.total_commande),0) as ca_journalier FROM commande
								   JOIN users ON commande.id_users = users.id_users
                                   WHERE DATE_FORMAT(date_commande, '%Y-%m')='".$date."' AND type_commande = 1 AND Gregory = 100.00)
                                   +
                                  (SELECT IFNULL(SUM(commande.total_commande),0) as ca_journalier_penalty FROM commande
								   JOIN users ON commande.id_users = users.id_users
	...
	public function getCAmonthSupplement_Optical_Service($date){
        ...
        $query = $this->db->query('SELECT SUM(commande.tarif_supplement) AS total_supplement FROM commande
		JOIN users ON commande.id_users = users.id_users
		WHERE DATE_FORMAT(date_commande, "%m-%Y") = "'.$date.'" AND Optical_Service = 100.00');
	...
	public function getCAmonth_Optical_Service($date){
        ...
        $query = $this->db->query("SELECT (SELECT IFNULL(SUM(commande.total_commande),0) as ca_journalier FROM commande
								   JOIN users ON commande.id_users = users.id_users
                                   WHERE DATE_FORMAT(date_commande, '%Y-%m')='".$date."' AND type_commande = 1 AND Optical_Service = 100.00)
                                   +
                                  (SELECT IFNULL(SUM(commande.total_commande),0) as ca_journalier_penalty FROM commande
								   JOIN users ON commande.id_users = users.id_users
	...
	public function getCAdaySupplement_Samuel(){
        ...
        $query = $this->db->query('SELECT SUM(commande.tarif_supplement) AS total_supplement FROM commande
		JOIN users ON commande.id_users = users.id_users
		WHERE (commande.date_commande > "'.$date.'" AND commande.date_commande < "'.$date_end.'") AND Samuel = 100.00');
	...
	public function getCAday_Samuel(){
        ...
        $query = $this->db->query("SELECT (SELECT IFNULL(SUM(commande.total_commande),0) as ca_journalier FROM commande
								   JOIN users ON commande.id_users = users.id_users
                                   WHERE (commande.date_commande > '".$date."' AND commande.date_commande < '".$date_end."') AND type_commande = 1 AND Samuel = 100.00)
                                   +
                                  (SELECT IFNULL(SUM(commande.total_commande),0) as ca_journalier_penalty FROM commande
								   JOIN users ON commande.id_users = users.id_users
	...
	public function getCAdaySupplement_Daniel(){
        ...
        $query = $this->db->query('SELECT SUM(commande.tarif_supplement) AS total_supplement FROM commande
		JOIN users ON commande.id_users = users.id_users
		WHERE (commande.date_commande > "'.$date.'" AND commande.date_commande < "'.$date_end.'") AND Daniel = 100.00');
	...
	public function getCAday_Daniel(){
        ...
        $query = $this->db->query("SELECT (SELECT IFNULL(SUM(commande.total_commande),0) as ca_journalier FROM commande
								   JOIN users ON commande.id_users = users.id_users
                                   WHERE (commande.date_commande > '".$date."' AND commande.date_commande < '".$date_end."') AND type_commande = 1 AND Daniel = 100.00)
                                   +
                                  (SELECT IFNULL(SUM(commande.total_commande),0) as ca_journalier_penalty FROM commande
								   JOIN users ON commande.id_users = users.id_users
	...
	public function getCAdaySupplement_Gregory(){
        ...
        $query = $this->db->query('SELECT SUM(commande.tarif_supplement) AS total_supplement FROM commande
		JOIN users ON commande.id_users = users.id_users
		WHERE (commande.date_commande > "'.$date.'" AND commande.date_commande < "'.$date_end.'") AND Gregory = 100.00');
	...
	public function getCAday_Gregory(){
        ...
        $query = $this->db->query("SELECT (SELECT IFNULL(SUM(commande.total_commande),0) as ca_journalier FROM commande
								   JOIN users ON commande.id_users = users.id_users
                                   WHERE (commande.date_commande > '".$date."' AND commande.date_commande < '".$date_end."') AND type_commande = 1 AND Gregory = 100.00)
                                   +
                                  (SELECT IFNULL(SUM(commande.total_commande),0) as ca_journalier_penalty FROM commande
								   JOIN users ON commande.id_users = users.id_users
	...
	public function getCAdaySupplement_Optical_Service(){
        ...
        $query = $this->db->query('SELECT SUM(commande.tarif_supplement) AS total_supplement FROM commande
		JOIN users ON commande.id_users = users.id_users
		WHERE (commande.date_commande > "'.$date.'" AND commande.date_commande < "'.$date_end.'") AND Optical_Service = 100.00');
	...
	public function getCAday_Optical_Service(){
        ...
        $query = $this->db->query("SELECT (SELECT IFNULL(SUM(commande.total_commande),0) as ca_journalier FROM commande
								   JOIN users ON commande.id_users = users.id_users
                                   WHERE (commande.date_commande > '".$date."' AND commande.date_commande < '".$date_end."') AND type_commande = 1 AND Optical_Service = 100.00)
                                   +
                                  (SELECT IFNULL(SUM(commande.total_commande),0) as ca_journalier_penalty FROM commande
								   JOIN users ON commande.id_users = users.id_users
    ...
    public function updateCommandeSend($last_date_commande,$magasin=0){
		...
		if($magasin != 0)
			if($magasin == 1)
				$this->db->where("((id_type_generation_verre <> 5 AND id_type_generation_verre <> 23) OR id_verre NOT IN (SELECT id_verre FROM verres_traduction))");
			elseif($magasin == 2)
				$this->db->where("(id_type_generation_verre = 5 OR id_type_generation_verre = 23) AND id_verre IN (SELECT id_verre FROM verres_traduction)");
			elseif($magasin == 4)
	...
	public function getCommandeJournalierePDF($date="", $magasin = 1, $envoi_chine = 0){
        ...
        $sql_add = "AND ((c.id_type_generation_verre <> 5 AND c.id_type_generation_verre <> 23) OR (c.id_type_generation_verre = 23 AND c.id_indice_verre = 6 ) OR c.id_verre NOT IN (SELECT id_verre FROM verres_traduction)) ";
        $date_add = "date_commande <='".$date."'";
        $email_send = " AND email_send = 0";
        $order_by = "c.id_commande";

        if($magasin == 2){
            $sql_add = "AND (c.id_type_generation_verre = 5 OR (c.id_type_generation_verre = 23 AND c.id_indice_verre != 6)) AND c.id_verre IN (SELECT id_verre FROM verres_traduction)";
    ...
    public function getCommandeJournaliere($date="", $magasin = 1, $envoi_chine = 0){
        ...
        $sql_add = "AND ((c.id_type_generation_verre <> 5 AND c.id_type_generation_verre <> 23) OR c.id_verre NOT IN (SELECT id_verre FROM verres_traduction))";
        $date_add = "date_commande <='".$date."'";
        $email_send = " AND email_send = 0";
        $order_by = "c.id_commande";

        if($magasin == 2){
            $sql_add = "AND (c.id_type_generation_verre = 5 OR c.id_type_generation_verre = 23) AND c.id_verre IN (SELECT id_verre FROM verres_traduction)";
            $order_by = "v.libelle_verre";
        }
    ...
    public function getCommandeRetard($type = 0){
        ...
        $query = $this->db->query('SELECT c.id_commande,c.id_users,c.id_type_generation_verre,date_commande,c.id_etat_commande, c.id_generation_verre, c.id_indice_verre, information_commande,reference_client,total_commande, penalty,libelle_etat_commande,generation_verre,type_generation_verre,libelle_verre,prix_verre,vt.traduction,c.id_verre
                                   ...
                                   WHERE (((c.id_type_generation_verre <> 5 AND c.id_type_generation_verre <> 23) AND c.id_etat_commande = 5 AND cp.date_pointage IS NULL)
                                   OR ((c.id_type_generation_verre = 5 OR c.id_type_generation_verre = 23) AND date_commande < "'.date('Y-m-d 23:59:59', strtotime('yesterday')).'" AND date_commande > "'.date('Y-m-d 00:00:00', strtotime('first day of last month')).'" AND c.id_etat_commande != 6 AND cp.date_pointage IS NULL))
    ...
    public function getEtiquette(){
        $query = $this->db->query("SELECT c.id_commande,id_users,information_commande,reference_client,libelle_verre,cote,date_commande
                               ...
                               AND id_etat_commande < 6
                               AND (c.id_type_generation_verre = 5 OR c.id_type_generation_verre = 23) AND c.id_indice_verre <> 6 AND c.id_generation_verre <> 14
                               ORDER BY ordre");
  ...
  public function getEtiquetteFabrication(){
		$query = $this->db->query("SELECT c.id_commande, c.id_indice_verre, c.origine_commande, id_users,information_commande,information_certificat,reference_client,libelle_verre,cote,date_commande,c.id_indice_verre,c.id_generation_verre
                               ...
                               WHERE date_click <= '".date('Y-m-d')."'
                               AND id_etat_commande < 6
                               AND ((c.id_type_generation_verre <> 5 AND c.id_type_generation_verre <> 23) OR c.id_indice_verre = 6 OR c.id_generation_verre = 16 OR c.id_generation_verre = 19)
                               OR (c.id_generation_verre = 9 AND c.id_indice_verre = 7)
    ...
    public function getCertificat(){
        $query = $this->db->query("SELECT c.id_commande,id_users,information_commande,information_certificat,reference_client,libelle_verre,cote,date_commande,c.id_indice_verre,c.id_generation_verre
                               ...
                               AND id_etat_commande < 6
                               AND ((c.id_type_generation_verre <> 5 AND c.id_type_generation_verre <> 23) OR c.id_indice_verre = 6 OR c.id_generation_verre = 14 OR c.id_generation_verre = 16)
                               OR (c.id_generation_verre = 9 AND c.id_indice_verre = 7)
                               GROUP BY c.id_commande
	...
	public function getCertificatStock($id_commande){
		$query = $this->db->query("SELECT c.id_commande,id_users,information_commande,information_certificat,reference_client,libelle_verre,date_commande,c.id_indice_verre,c.id_generation_verre
                               FROM commande c
                               INNER JOIN verres v ON v.id_verre = c.id_verre
                               WHERE  c.id_commande = '".$id_commande."' AND c.information_certificat != ''
                               AND id_etat_commande < 6
                               AND  c.id_generation_verre = 23
                               GROUP BY c.id_commande
                               ORDER BY date_commande DESC");
		 if ($query && $query->num_rows() > 0){
            return $query->result();
    ...
    public function getUpdateCommande($type_commande = 1){
        ...
        if($type_commande == 1) {
            $sql = 'AND (((c.id_type_generation_verre <> 5 AND c.id_type_generation_verre <> 23) OR c.id_indice_verre = 6 OR c.id_generation_verre = 16 OR c.id_generation_verre = 19) OR (c.id_generation_verre = 9 AND c.id_indice_verre = 7))';
        }
        else {
            $sql = 'AND (c.id_type_generation_verre = 5 OR c.id_type_generation_verre = 23) AND c.id_generation_verre <> 16 AND c.id_generation_verre <> 19 AND c.id_indice_verre != 6 AND c.id_indice_verre != 7';
        }
  ...
  public function getCommandeStockJournalier(){
    $query = $this->db->query("SELECT information_commande,libelle_verre, count(id_commande) as nb_commande
                               ...
                               INNER JOIN indice_verre iv ON iv.id_indice_verre = c.id_indice_verre
                               INNER JOIN type_generation_verre tgv ON tgv.id_type_generation_verre = c.id_type_generation_verre
                               WHERE (c.id_type_generation_verre = 5 OR c.id_type_generation_verre = 23)
