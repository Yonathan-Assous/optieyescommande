...
<script>
    $(document).ready(function() {
        ...
        var table = $('#datatable').DataTable({
            ajax: { url: "/admin/ancienne_commande_montures_ajax", dataSrc: 'aaData' },
            deferRender: true,
            ordering: false,
            serverSide: true,
            columnDefs: [{
        ...
        $(document).on("change", "#recherche_num_magasin", function(){
            ...
            if(value != '') {
                table.ajax.url('/admin/ancienne_commande_montures_ajax?id_users='+value).load();
            }
            else {
                table.ajax.url('/admin/ancienne_commande_montures_ajax').load();
            }
                ...
                $.ajax({
                    ...
                    success: function(data){
                        if($("#recherche_num_magasin").val() != '') {
                            table.ajax.url('/admin/ancienne_commande_montures_ajax?id_users='+$("#recherche_num_magasin").val()).load();
                        }
                        else {
                            table.ajax.url('/admin/ancienne_commande_montures_ajax').load();
                        }
