       ...
       if($k == 'order' && isset($orderbag['pair_order'])) {
        ...
        if($user_info->restriction_compte == 0){
            ...
            if($recap_commande['type_commande'] > 1) {
                ...
                if(isset($pair_order_recap)) {
                    echo '<div class="total"><span>Total</span><strong><del><b class="calculatedTotal" data-base="'.( $recap_commande['total_commande'] * $quantite_commande ).'">'.(( $recap_commande['total_commande'] * $quantite_commande )+$prixmonture).'</b> €</del></strong></div>';
                }
                else {
                    echo '<div class="total"><span>Total</span><strong><del><b class="calculatedTotal" data-base="'.$total_commande.'">'.($total_commande+$prixmonture).'</b> €</del></strong></div>';
                }
