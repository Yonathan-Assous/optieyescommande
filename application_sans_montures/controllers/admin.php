...
class admin extends MY_Controller {
    ...
    */
        ...
        if($this->input->is_ajax_request()){
            ...
            if($users !== false)
                foreach($users as $key => $user){
                    ...
                    $commander = '<a href="/admin/passer_commande/'.$user->id_users.'" class="btn btn-warning" '. ($user->valid_mandat == 0 ? 'disabled' : '').'>Verres</a> <a href="/admin/commande_lentilles/'.$user->id_users.'" class="btn btn-warning" '. ($user->valid_mandat == 0 ? 'disabled' : '').'>Lentilles</a>';
    ...
    public function getGrilleFabrication(){
        if($this->input->is_ajax_request()){
            ...
            if(isset($data['pair_order'])) {
                $data['pair_order'] = $this->m_commande->getCommandeById($data['pair_order'], $id_users = false, $pair = true)[0];
            }
    ...
    public function getIndiceVerre(){
        if($this->input->is_ajax_request()){
            ...
            if(isset($pair_order_id)) {
                $pair_order = $this->m_commande->getCommandeById($pair_order_id, $id_users = false, $pair = true)[0];
            }
            ...
            if(($result = $this->m_generation_verre->getIndiceVerre($this->input->post('id_generation_verre')))!== false){
                ...
                if(isset($pair_order_id) && false !== $pair_order_id) {
                    ...
                    foreach ($result as $indice) {
                        ...
                        //echo '<pre>'; var_dump($fabrication); echo '</pre><br />';
                        //echo '<pre>'; var_dump($pair_corrections->verre); echo '</pre><br />---<br /><br /><br />';
    ...
    public function correction_stock(){
        $dataVerre['id_generation_verre'] = '9,12,13,14,15,16,17,19,20,21';
    ...
    public function variations_facture_clients_ajax(){
        ...
        if($data !== false){
            ...
            foreach($data as $users) {
                ...
                $lignZero[$users->mois] = '<b>'.$users->total. ' HT</b><br />('.($users->total_lentilles > 0 ? $users->total_lentilles : '0.00').' Lentilles)';
