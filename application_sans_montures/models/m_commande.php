...
class m_commande extends CI_Model {
	...
	public function getOldOrder($startLimit,$offsetLimit,$search, $id_users){
		...
		if($search != "" || $id_users !== false){
		 ...
		 $query = $this->db->query("SELECT c.id_users,c.id_commande,c.intitule_bl, c.tarif_express, c.penalty, c.ancienne_commande, c.total_commande, c.tarif_express, date_commande,reference_client,date_update_commande,type_commande,nom_magasin,nom_societe,commentaire
                                    FROM ".$this->table." c
                                    INNER JOIN users u ON c.id_users = u.id_users 
                                    LEFT JOIN commande_commentaire cc ON cc.id_commande = c.id_commande
    ...
    public function getAllCommandeByMonth($data){
        ...
        if ($query && $query->num_rows() > 0){
          foreach($query->result() as $result){
            ...
            $query = $this->db->query("SELECT l.name as lens_name, c.id_commande,c.id_users, c.type_commande, c.penalty, date_commande,c.tarif_express,reference_client,total_commande,generation_verre,taux_tva,libelle_verre_personnalise, 0 as TarifLivraison,intitule_bl, c.id_users, date_update_commande, tarif_packaging
                                       FROM ".$this->table." c
    ...
    public function getAllCommandeByMonthAndUser($date, $excl = null, $user = null){
        ...
        $query = $this->db->query("SELECT COALESCE(total_stock, 0)  + COALESCE(total_fabrique, 0) + COALESCE(total_lentilles, 0) + COALESCE(total_express, 0) - COALESCE(total_reductions, 0) as total,total_stock,total_fabrique,total_lentilles, total_express, c.type_commande, c.tarif_express, c.penalty, c.date_commande,c.id_users, COALESCE(total_reductions, 0) as reduction,cp,nom_magasin,id_grille_tarifaire,nom_societe,intitule_bl, 0 as tarif_liv, COALESCE(exp_stock) AS expeditions_stock, COALESCE(exp_fabric) AS expeditions_fabric
           FROM ".$this->table." c
           LEFT JOIN generation_verre gv ON gv.id_generation_verre = c.id_generation_verre
           INNER JOIN users u ON u.id_users = c.id_users
           LEFT JOIN
    ...
    public function getCommandeRetard($type = 0){
        ...
        $query = $this->db->query('SELECT c.id_commande,c.id_users,c.id_type_generation_verre,date_commande,c.id_etat_commande, c.id_generation_verre, c.id_indice_verre, information_commande,reference_client,total_commande, penalty,libelle_etat_commande,generation_verre,type_generation_verre,libelle_verre,prix_verre,vt.traduction,c.id_verre
                                   ...
                                   WHERE ((c.id_type_generation_verre <> 5 AND c.id_etat_commande = 5 AND cp.date_pointage IS NULL)
                                   OR (c.id_type_generation_verre = 5 AND date_commande < "'.date('Y-m-d 23:59:59', strtotime('yesterday')).'" AND date_commande > "'.date('Y-m-d 00:00:00', strtotime('first day of last month')).'" AND c.id_etat_commande != 6 AND cp.date_pointage IS NULL))
  ...
  public function getEtiquetteFabrication(){
		$query = $this->db->query("SELECT c.id_commande, c.id_indice_verre, c.origine_commande, id_users,information_commande,information_certificat,reference_client,libelle_verre,cote,date_commande,c.id_indice_verre,c.id_generation_verre
                               ...
                               WHERE date_click <= '".date('Y-m-d')."'
                               AND id_etat_commande < 6
                               AND (c.id_type_generation_verre <> 5 OR c.id_indice_verre = 6 OR c.id_generation_verre = 16 OR c.id_generation_verre = 19)
    ...
    public function getCertificat(){
        $query = $this->db->query("SELECT c.id_commande,id_users,information_commande,information_certificat,reference_client,libelle_verre,cote,date_commande,c.id_indice_verre,c.id_generation_verre
                               ...
                               AND id_etat_commande < 6
                               AND (c.id_type_generation_verre <> 5 OR c.id_indice_verre = 6 OR c.id_generation_verre = 14 OR c.id_generation_verre = 16)
                               OR (c.id_generation_verre = 9 AND c.id_indice_verre = 7)
                               GROUP BY c.id_commande
    ...
    public function getUpdateCommande($type_commande = 1){
        ...
        if($type_commande == 1) {
            $sql = 'AND ((c.id_type_generation_verre <> 5 OR c.id_indice_verre = 6 OR c.id_generation_verre = 16 OR c.id_generation_verre = 19) OR (c.id_generation_verre = 9 AND c.id_indice_verre = 7))';
        }
        else {
            $sql = 'AND c.id_type_generation_verre = 5 AND c.id_generation_verre <> 16 AND c.id_generation_verre <> 19 AND c.id_indice_verre != 6 AND c.id_indice_verre != 7';
        }
  ...
  public function getNbIntituleBl($intitule_bl){   
    ...
    public function getAllCommandeByYearAndUser($data){
        ...
        $query = $this->db->query("SELECT COALESCE(total_stock, 0) + COALESCE(total_fabrique, 0) + COALESCE(total_lentilles, 0) + COALESCE(total_express, 0) - COALESCE(total_reductions, 0) as total,total_stock,total_fabrique,total_lentilles, c.id_users, DATE_FORMAT(c.date_commande, '%c') as mois, nom_societe, nom_magasin
                                   FROM commande c
                                   LEFT JOIN generation_verre gv ON gv.id_generation_verre = c.id_generation_verre
                                   INNER JOIN users u ON u.id_users = c.id_users
                                   LEFT JOIN
