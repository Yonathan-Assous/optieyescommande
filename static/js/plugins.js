...
(function($, window, document){
	...
	$.fn[PLUGIN_NAME].defaults = {
		...
		dataTable: {"sExtends": "pdf",
            "sPdfOrientation": "landscape",
            "sPdfMessage": "Your custom message would go here."},	// dataTable options
		tableTools: {
			display               : false,										// Show tableTools?
			buttons               : ['pdf'],				// Which buttons to show 'copy', 'csv', 'xls', 'pdf'
...
(function($, window, document){
    ...
    $.fn[PLUGIN_NAME] = function(options){
        ...
        return  $(this)
		.each(function(){
            var $this = $(this).on('pageSwitch', function (event, $newStep, newIndex) {options.onPageSwitch.call($this, $newStep, newIndex);});
            ...
            var goToIndex = function(newIndex){
                ...
                if($newStepLink.hasClass('success')){
                    nbSuccess--;
                }
                ...
                if (newIndex == length - 1) {
                    ...
                    $el.next.show();
                    $el.finish.show().css('display', 'none');
                }
				...
				$this.trigger('pageSwitch', [$newStep, newIndex]);
            },
            index = 0,
            length = 0;
			...
			$this.on('goToIndex', function (event, index) {
				goToIndex(index);
			});
            // ! Set up steps links
            $el.a_list.click(function(e){
                ...
                if((nbSuccess == parent.index()-1) || $(this).hasClass('success'))
                    goToIndex(parent.index());
            });
				...
				open: function(){
					if (!init) {
						...
						$pwOk.click(function(){
							$.ajax({
								type: "POST",
								url: $pwPopup.find('form').attr('action'),
								data: $pwPopup.find('form').serialize(),
