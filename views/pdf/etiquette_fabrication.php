			...
			if(isset($etiquette) && $etiquette !== false){
				...
				foreach($etiquette as $value){
					...
					if(isset($info_commande['verre']['miroir']['type_miroir'])) {
						...
						$value->libelle_verre = str_replace('Durci', 'HMC Face interne', $value->libelle_verre).' '.$options_miroir[$info_commande['verre']['miroir']['type_miroir']];
					}
					...
					echo '<tr>
									<td style="text-align:right;">
										<table>
											<tr>
												<td style="font-size:7px;text-align:left;width:140px;">
                          Date commande : <b>'.date("d/m/Y",strtotime($value->date_commande)).'</b><br>
													Reference Client : <b>'.$value->reference_client.'</b>
												</td>
												<td style="font-size:7px;text-align:right;width:105px">
													<b>Ref Optieyes : CR'.$value->id_commande."-".$value->id_users.'</b>
												</td>
											</tr>
										</table>
									</td>
					...
					if($compteur_etiquette == 2) {
						...
						if (is_array(json_decode($value->information_certificat, true))) {
							echo '<tr>
											<td style="width:250px;text-align:center; font-size: 12px;">CERTIFI�</td>
										</tr>';
						}
